...
- Blocks widen automatically as you type longer text, up to a maximum width. Process blocks use their longest line and allow horizontal scrolling for text beyond the limit. Deleting text keeps the current width, and any extra width you set manually is preserved. For more room, select the block and drag either blue grip at its left or right edge. Width changes can be undone and are preserved when the program is saved.
...
- The left palette and right runtime sidebar scroll independently. On desktop, drag either sidebar divider to resize it, or focus the divider and use Left/Right arrow keys. Use the header buttons to hide either sidebar. On screens up to 1100 pixels wide, **Blocks**, **Canvas**, and **Console** tabs give each panel the full workspace above the Output drawer. Canvas opens first and keeps Run, Step, and Restart/Stop visible; Console has the full controls, including Auto Step and its speed slider. Selecting a block in Blocks returns to Canvas for placement. The Output drawer stays available in every tab.
